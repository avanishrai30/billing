...
ENV_FILE="/etc/vc-organic/production.env"
RELEASE_ROOT="/opt/vc-organic-production-releases"
CURRENT_LINK="/opt/vc-organic-production-current"
PREVIOUS_LINK="/opt/vc-organic-production-previous"
ECOSYSTEM_FILE="ops/ecosystem.production.config.js"
API_PROCESS="vc-organic-api-production"
WEB_PROCESS="vc-organic-web-production"
API_PORT="8181"
WEB_PORT="3001"
API_HEALTH_URL="http://127.0.0.1:${API_PORT}/health"
WEB_HEALTH_URL="http://127.0.0.1:${WEB_PORT}"
LOCK_FILE="/tmp/vc-organic-production-deploy.lock"
...
install_and_build() {
  local release_dir="$1"
  source_env
  export NEXT_PUBLIC_API_BASE_URL="https://api.vcorganics.com"
  export NEXT_PUBLIC_API_URL="https://api.vcorganics.com"
...
main() {
  ...
  APP_RUNTIME_ENV="${APP_RUNTIME_ENV}" node "${release_dir}/scripts/deploy-check.js" --env production --no-health --no-pm2 --no-cwd --no-port
  ...
  pm2 save
  retain_releases
  echo "Production deployed ${commit}"
}
