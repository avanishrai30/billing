...
router.get('/', verifyJWT, requirePermission('suppliers.view'), async (req, res) => {
  const { db } = getContext();
  try {
    const suppliers = await db.collection('suppliers').find().toArray();
    res.json(suppliers); // Return array directly for backward compatibility
  } catch (err) {
    res.status(500).json({ success: false, error: { code: "SERVER_ERROR", message: "Failed to fetch suppliers" } });
  }
});

// GET /api/v1/suppliers/:id - Fetch supplier by ID
router.get('/:id', verifyJWT, requirePermission('suppliers.view'), async (req, res) => {
  const { db } = getContext();
  try {
    ...
    res.status(500).json({ success: false, error: { code: "SERVER_ERROR", message: "Failed to fetch supplier" } });
  }
});

// POST /api/v1/suppliers - Create or update supplier
router.post('/', verifyJWT, requireAnyPermission(['suppliers.create', 'suppliers.update']), async (req, res) => {
  const { db, io } = getContext();
  const supplier = req.body;
  if (!supplier.name || !supplier.contact) {
    return res.status(400).json({ success: false, error: { code: "INVALID_INPUT", message: "Supplier name and contact required" } });
  }
  ...
  try {
    ...
    res.status(500).json({ success: false, error: { code: "SERVER_ERROR", message: "Failed to save supplier" } });
  }
});

// PATCH /api/v1/suppliers/:id - Partial update
router.patch('/:id', verifyJWT, requirePermission('suppliers.update'), async (req, res) => {
  const { db, io } = getContext();
  ...
  try {
    ...
    res.status(500).json({ success: false, error: { code: "SERVER_ERROR", message: "Failed to update supplier" } });
  }
});

// DELETE /api/v1/suppliers/:id - Delete supplier
router.delete('/:id', verifyJWT, requirePermission('suppliers.delete'), async (req, res) => {
  const { db, io } = getContext();
  const supId = req.params.id;
  try {
    ...
    res.status(500).json({ success: false, error: { code: "SERVER_ERROR", message: "Failed to delete supplier" } });
  }
});
