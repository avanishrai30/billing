...
router.get('/summary', verifyJWT, requireSuperAdmin, async (req, res) => {
  try {
    const summary = await adminCleanupService.getDomainSummary();
...
router.post('/:domain/query', verifyJWT, requireSuperAdmin, async (req, res) => {
  try {
    ...
    const result = await adminCleanupService.queryDomainRecords(domain, filters, pagination);
...
router.get('/operations', verifyJWT, requireSuperAdmin, async (req, res) => {
  try {
    ...
    const result = await adminCleanupService.listOperations(limit, skip);
    res.json({ success: true, ...result });
  } catch (err) {
...
router.get('/operations/:operationId', verifyJWT, requireSuperAdmin, async (req, res) => {
  try {
    const operation = await adminCleanupService.getOperation(req.params.operationId);
...
router.post('/operations/:operationId/rollback', verifyJWT, requireSuperAdmin, async (req, res) => {
  try {
    ...
    res.status(400).json({ success: false, error: { code: 'ROLLBACK_ERROR', message: err.message } });
  }
});
